                            ...
                            <div class="bg-white rounded-xl shadow-md p-5 hover:shadow-xl transition duration-300">
                                <div class="flex items-center gap-4 mb-4">
                                    <img src="{{ $feedback->user->image ?? asset('images/default-avatar.png') }}" 
                                        alt="User Avatar" 
                                        class="w-20 h-20 rounded-full object-cover border-2 border-gray-300 shadow-sm overflow-hidden"
    ...
    <div class="relative h-screen overflow-hidden">
        ...
        <div class="relative z-20 flex h-full">
            <main class="flex-1 p-4 md:p-8 space-y-8 overflow-y-auto">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    @foreach ([
                        ...
                        <div class="group relative overflow-hidden bg-white/10 backdrop-blur-lg p-6 rounded-2xl shadow-xl transition-all duration-300 ease-in-out hover:shadow-2xl hover:-translate-y-2 border border-white/10">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-br {{ $card['color'] }} opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl -z-10"></div>
                            ...
                            <h3 class="text-xl font-bold text-white group-hover:text-white transition-colors duration-300">{{ $card['title'] }}</h3>
                            <p class="text-gray-300 mt-2 group-hover:text-white/90 transition-colors duration-300">{{ $card['desc'] }}</p>
                            
                            <div class="flex justify-between items-center mt-6 pt-4 border-t border-white/10">
                                <button class="text-white/80 font-medium transition hover:text-white group-hover:scale-105 flex items-center gap-1" onclick="openModal('{{ $card['feature'] }}')">
                                    <span>More Info</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </button>
                                <a href="{{ $card['link'] }}" class="flex items-center gap-1 text-white font-medium transition hover:text-white group-hover:scale-105">
                ...
                <div class="bg-white/10 backdrop-blur-lg p-8 rounded-2xl shadow-xl mt-12 border border-white/10">
                    <div class="flex items-center justify-between mb-8">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-gray-300">({{ count($feedbacks) }} reviews)</span>
                        </div>
                    </div>
                        ...
                        @forelse($feedbacks as $feedback)
                            <div class="bg-white/10 backdrop-blur-md rounded-xl p-6 hover:bg-white/20 transition duration-300 border border-white/10">
                                <div class="flex items-center gap-4 mb-4">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-lg text-white">{{ $feedback->user->name }}</h4>
                                        <div class="flex items-center text-yellow-400 mt-1">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <svg class="w-4 h-4 {{ $i <= $feedback->rating ? 'text-yellow-400' : 'text-gray-500' }}" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <p class="text-gray-200 leading-relaxed">{{ $feedback->feedback }}</p>
                                <div class="mt-4 text-gray-400 text-sm">{{ $feedback->created_at->diffForHumans() }}</div>
                            </div>
                        @empty
        ...
        <script>
            ...
            const featureImages = {
                ...
                "create-group": [
                    "{{ asset('images/cg1.png') }}",
                    "{{ asset('images/cg2.png') }}",
                    "{{ asset('images/cg3.png') }}"
                ],
                "group-chat": [
                    "{{ asset('images/gc1.png') }}",
                    "{{ asset('images/gc2.png') }}",
                    "{{ asset('images/gc3.png') }}"
                ],
                meetings: [
                    "{{ asset('images/m1.png') }}",
                    "{{ asset('images/m2.png') }}",
