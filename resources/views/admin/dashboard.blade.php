...
<body>
    <div class="container">
        ...
        <main>
            <header>
                <h1>Welcome, Admin</h1>
                <span class="last-login" id="lastLogin">Last Login: {{ now()->format('F j, Y') }} - {{ now()->format('H:i:s') }}</span>
            </header>
            <section>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @endphp
                                <img src="{{ asset($image) }}" alt="User Image" style="width: 50px; height: 50px; border-radius: 50%;">
                            </td>
                            <td>
                                <a href="{{ route('users.edit', $user->id) }}" class="button button-edit" style="margin-right: 10px;">
                                    Edit
                                </a>
                                <form id="delete-form-{{ $user->id }}" action="{{ route('users.destroy', $user->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="button button-delete" onclick="confirmDeleteWithToast({{ $user->id }})">
                                        Delete
                                    </button>
                                </form>
